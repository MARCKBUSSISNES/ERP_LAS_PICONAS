...
window.imprimirTicketDespachoAgrupado = function(despacho){
    ...
    var itemsRows = (despacho.items || []).map(function(it){
        ...
        return `
            <tr>
                <td class="prod">
                    <div class="prod-name">${escapeHtml(it.producto || "")}</div>
                    <div class="prod-sub">Sucursales: ${escapeHtml(sucTxt || "-")}</div>
                </td>
    ...
    var clientesRows = (despacho.clientes || []).map(function(c){
        ...
        return `
          <tr>
            <td class="left">
              <div class="prod-name">${escapeHtml(c.clienteNombre || "")}</div>
              <div class="prod-sub">${escapeHtml(itemsTxt || "Sin detalle")}</div>
            </td>
            <td class="right total-line">${escapeHtml(c.envioId || "")}</td>
          </tr>
        `;
    }).join("");
    ...
    win.document.write(`
        ...
        <body>
            ...
            <div class="box">
                <div class="meta-row"><span class="bold">Ruta:</span><span>${escapeHtml(despacho.despachoId || "")}</span></div>
                <div class="meta-row"><span class="bold">Fecha:</span><span>${fechaStr}</span></div>
                <div class="meta-row"><span class="bold">Hora:</span><span>${horaStr}</span></div>
            </div>
            ...
            <div class="box">
                <div class="label">TRANSPORTE</div>
                <div><span class="bold">Vehículo:</span> ${escapeHtml(despacho.vehiculoPlaca || "")}</div>
                <div><span class="bold">Piloto:</span> ${escapeHtml(despacho.vehiculoPiloto || "")}</div>
                <div><span class="bold">Generado por:</span> ${escapeHtml(despacho.creadoPor || "")}</div>
            </div>

            <div class="box">
                <div class="label">SUCURSALES INCLUIDAS</div>
                ${(despacho.clientes || []).map(function(c){ return '<div>• ' + escapeHtml(c.clienteNombre || '') + ' <span class="small">(' + escapeHtml(c.envioId || '') + ')</span></div>'; }).join('') || '<div class="small">Sin sucursales</div>'}
            </div>
